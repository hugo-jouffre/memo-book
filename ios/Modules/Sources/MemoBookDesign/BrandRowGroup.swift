...
public struct BrandRow: View, Identifiable {
    ...
    /// Une ligne qu'on corrige sur place.
    private func editableRow(_ field: Editable) -> some View {
        ...
        .padding(.vertical, MemoBookSpacing.xs + 4)
        .frame(minHeight: minimumHeight)
        .background(isEditing ? MemoBookColor.background : Color.clear)
        .contentShape(.rect)
        ...
        // faire défiler la page (`scrollDismissesKeyboard`), toucher un autre
        // champ. Rien à valider.
    ...
    /// Le crayon : sans lui, rien ne dit qu'une ligne se corrige. La maquette
    /// ne le montre pas sur ces lignes-là — c'est le prix d'une valeur qu'on
    /// peut toucher, et il est signalé dans la fiche écran.
