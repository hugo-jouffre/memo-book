...
| L'agent de conversation | Côté serveur, sur le motif `Transcriber` / `Redactor`, trois paliers : `AnthropicResponder` (Claude Sonnet 5, **prompt système = `agents/agent-conversation.md`**, sortie JSON contrainte) dès qu'il y a une clé ; `HeuristicResponder` (le portage des règles de `LocalMemoResponder`) sans clé **et en repli à chaque tour qui tombe** ; `FakeResponder` en CI et en `PIPELINE_MODE=fake`. Le modèle écrit des phrases : le rythme (`composeBeats`), le catalogue de puces et la rose/épine/graine restent au code, et une réponse hors contrat est refusée — le repli parle, et `model` dit qui. Relecture : `npm run conversation:eval` (22/09/2026) |
...
| # | Sujet |
|---|---|
| T184 | **La notification de relance n'existe pas encore** — c'est le rythme du récit (« Tous les 2 jours ») qui devrait la cadencer. (Noté dans la PR #40 sous le numéro T161, repris depuis par § 24.2 ; la fiche ne l'avait pas.) |
...
| Profil — Exporter mes données | l'icône orange au centre, sans contour, comme les deux lignes d'en dessous | le halo orange à 50 % que `Export data.svg` portait autour du signe est retiré du fichier source et de l'asset importé ; le signe seul se teinte en `warning`, comme `IconExit` et `IconCross` se teintent en encre |
