...
```
                     ┌─ payWithWallet: false ─→ PaymentIntent ─→ feuille ─→ webhook ─┐
POST /memos/:id/orders                                                                ├─→ submitted
                     └─ payWithWallet: true ──→ écriture au registre ─────────────────┘
...
webhook pour une carte, écriture de registre pour la cagnotte.
...
compte, carnet, commande, paiement carte 4242, attente du webhook, puis
recharge de cagnotte et refus pour solde insuffisant. C'est le seul chemin qui
exerce la vraie signature de webhook — `test/payments.test.ts` passe par
`FakePaymentGateway` et ne peut pas la couvrir.
