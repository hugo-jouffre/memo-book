...
import { MEMORY_UPGRADE_MONTHLY_CENTS } from "./subscriptionCatalog.js";
 ...
 * souvenirs sont le budget mensuel de quelqu'un qui raconte **déjà** — un
 * abonné. Elles se rechargent tous les mois, et se relèvent contre
 * 3,99 €/mois.
 *
...
/** Ce que chaque palier ouvre par mois. */
export const MEMORY_ALLOWANCE: Record<MemoryPlan, number> = {
  // ~300 minutes de vocal, ou 3 000 messages. Volontairement **haut** : cette
  // limite n'est pas un levier commercial, c'est un garde-fou contre l'usage
  // qui coûterait plus cher que l'abonnement. Quelqu'un qui raconte
  // normalement ne doit jamais la voir bouger.
  included: 3_000,
  // Quatre fois plus, pour qui raconte tous les jours à plusieurs.
  extended: 12_000,
};

/** La durée d'une période, en jours. Un mois glissant. */
const PERIOD_DAYS = 30;

/** Le prix mensuel du palier étendu, en centimes. Réexporté pour l'API. */
export const MEMORY_UPGRADE_CENTS = MEMORY_UPGRADE_MONTHLY_CENTS;
...
/**
 * Le début de période à retenir : celui qu'on a, ou aujourd'hui si le mois est
 * écoulé.
 *
 * **Glissant et non calendaire** : un compte ouvert le 28 aurait sinon trois
 * jours de budget pour son premier mois, ce qui se lit comme un bogue et ne se
 * rattrape par aucune explication.
 */
function currentPeriodStart(start: Date, now: Date): Date {
  ...
  // maintenant : le jour de renouvellement reste le même d'un mois à l'autre,
  // même après trois mois sans ouvrir l'app.
 ...
 * budget mensuel n'a pas besoin d'être exact à la seconde, il a besoin d'être
 * juste au moment où quelqu'un le regarde ou le consomme. Une tâche de plus
 * pour ça serait une pièce mobile de plus à surveiller.
 */
export async function readMemoryAllowance(
  ...
  return {
    ...
    upgradeMonthlyPrice: MEMORY_UPGRADE_CENTS,
  };
}
...
export async function consumeMemory(
  ...
  if (snapshot.used + cost > snapshot.allowance) {
    throw new HttpError(
      403,
      "Tu as atteint tes limites de souvenirs pour ce mois-ci. Étends-les pour continuer à raconter.",
